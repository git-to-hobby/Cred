...
# === TESSERACT CONFIG ===
_default_tesseract = shutil.which("tesseract") or r"C:\Program Files\Tesseract-OCR\tesseract.exe"
tesseract_cmd = (os.getenv("TESSERACT_CMD") or _default_tesseract).strip()
if os.path.exists(tesseract_cmd) or shutil.which(tesseract_cmd):
    resolved = shutil.which(tesseract_cmd) or tesseract_cmd
    pytesseract.pytesseract.tesseract_cmd = resolved
    logger.info("Tesseract found: %s", resolved)
else:
    logger.error("Tesseract NOT found at: %s", tesseract_cmd)
