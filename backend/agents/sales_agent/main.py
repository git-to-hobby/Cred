...
async def get_llm_sales_response(user_query: str, customer_id: str) -> str:
    ...
    system_prompt = f"""
    You are CredFlow Assistant, a friendly, persuasive, and knowledgeable loan sales executive for CredFlow Finance.
    ...
    3. **Sell:** Highlight the benefits of CredFlow Finance (quick process, competitive rates) even when discussing government schemes.
...
async def handle_sales(request: SalesRequest):
    ...
    if not pre_approved_offer:
        ...
        general_query_keywords = [
            "car", "home", "market", "rates", "advice", "should i", "recommend",
            "business", "type", "options", "compare", "scheme", "government",
            "subsidy", "exporter", "personal", "loan", "eligibility", "document",
            "emi", "interest", "apply", "need", "require", "check", "how",
        ]
        if user_message.strip() and (
            any(keyword in user_message.lower() for keyword in general_query_keywords)
            or len(user_message.strip()) > 20
        ):
